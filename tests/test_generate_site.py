...
def test_generate_site_builds_trees_and_pages(sandbox):
    ...
    assert (sandbox / "security" / "README.md").exists()
    assert (sandbox / "ai" / "README.md").exists()
    ai_pages = list((sandbox / "ai" / c.domain_slug("Agents & Harnesses")).glob("*.md"))
    sec_pages = list(
        (sandbox / "security" / c.domain_slug("Web Application Security")).glob("*.md")
    )
    assert ai_pages and sec_pages
    landing = (sandbox / "README.md").read_text(encoding="utf-8")
    assert "Top learnings" in landing


def test_legacy_entry_renders_without_scores(sandbox):
    _seed(sandbox)
    conf = c.load_config()
    legacy = c.load_pool("security")["entries"][0]
